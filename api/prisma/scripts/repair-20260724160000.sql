-- Repair script for failed migration:
--   20260724160000_gabinete_location_therapist_access
--
-- Run against production AFTER checking state (see README below).
-- Safe to re-run: skips steps that are already done.

-- ---------------------------------------------------------------------------
-- 1) gabinetes.location_id
-- ---------------------------------------------------------------------------
...
-- Assign each gabinete to the first active location (fallback: any location)
...
-- Abort with a clear error if gabinetes still have no location
SET @null_gabinetes := (
  SELECT COUNT(*) FROM `gabinetes` WHERE `location_id` IS NULL
);
SET @sql := IF(
  @null_gabinetes > 0,
  'SELECT ''ERROR: gabinetes without location_id — create at least one location first'' AS error',
  'SELECT ''gabinetes.location_id populated'' AS info'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  ...
  'SELECT ''skipped NOT NULL until locations exist'' AS info'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  ...
  'CREATE TABLE `therapist_locations` (
    ...
    PRIMARY KEY (`therapist_id`, `location_id`),
...
INSERT IGNORE INTO `therapist_locations` (`therapist_id`, `location_id`)
SELECT `u`.`id`, `l`.`id`
FROM `users` `u`
...
INSERT IGNORE INTO `therapist_locations` (`therapist_id`, `location_id`)
SELECT DISTINCT `therapist_id`, `location_id` FROM `patients`;
