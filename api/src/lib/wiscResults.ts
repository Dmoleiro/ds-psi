...
type WiscQiLookup = { qi: string; percentil: string; ic90: string; ic95: string }
type WiscGaiLookup = { gai: string; percentil: string; ic90: string; ic95: string }
type AgeBand = {
  years: number
  monthsFrom: number
  monthsTo: number
...
export const WISC_SCALE_SUMMARY_KEYS = WISC_SCALE_SUMMARY_ROWS.map((row) => row.key)
...
export function lookupWiscScaledScore(
  ...
  for (const [min, max, scaled] of ranges) {
...
export function computeWiscSomaPadronizados(
  ...
  for (const column of WISC_PADRONIZADO_COLUMNS) {
    ...
    totals[column.key] = complete ? formatWiscNumericTotal(values.reduce((a, b) => a + b, 0)) : ''
  }
  return totals
}
...
export function lookupWiscQi(tableId: string, soma: string): WiscQiLookup | null {
  ...
  return NORMS.qiTables[tableId]?.[String(Math.round(total))] ?? null
}
...
export function lookupWiscGai(soma: string): WiscGaiLookup | null {
  const total = parseWiscNumericCell(soma)
  if (total === null) return null
  return GAI_NORMS.gaiTable[String(Math.round(total))] ?? null
}
...
const MAX_AGE_LENGTH = 8

function sanitizeCell(value: unknown): string {
  if (typeof value !== 'string') return ''
  return value.trim().slice(0, MAX_CELL_LENGTH)
}

function sanitizeAgePart(value: unknown): string {
  if (typeof value === 'number' && Number.isFinite(value)) return String(value).slice(0, MAX_AGE_LENGTH)
  if (typeof value !== 'string') return ''
  return value.trim().slice(0, MAX_AGE_LENGTH)
}

function sanitizePadronizadoColumns(subtestKey: string, value: unknown): WiscPadronizadoColumns {
  const input = value && typeof value === 'object' ? (value as Record<string, unknown>) : {}
  const columns = emptyWiscPadronizadoColumns()
  for (const column of WISC_PADRONIZADO_COLUMNS) {
    if (isWiscPadronizadoEditable(subtestKey, column.key)) {
      columns[column.key] = sanitizeCell(input[column.key])
    }
  }
  return columns
}

function sanitizeSubtestResult(subtestKey: string, value: unknown): WiscSubtestResult {
  const input = value && typeof value === 'object' ? (value as Record<string, unknown>) : {}
  return {
    brutos: sanitizeCell(input.brutos),
    padronizado: sanitizePadronizadoColumns(subtestKey, input.padronizado),
  }
}

function sanitizeScaleSummaryManualFields(value: unknown): WiscScaleSummaryRow {
  const input = value && typeof value === 'object' ? (value as Record<string, unknown>) : {}
  const legacyIc = sanitizeCell(input.intervaloConfianca)
  return {
    resultado: '',
    qi: sanitizeCell(input.qi),
    percentil: sanitizeCell(input.percentil),
    intervaloConfianca90: sanitizeCell(input.intervaloConfianca90) || legacyIc,
    intervaloConfianca95: sanitizeCell(input.intervaloConfianca95),
  }
}

function sanitizeGaiSummaryManualFields(value: unknown): WiscGaiSummary {
  const input = value && typeof value === 'object' ? (value as Record<string, unknown>) : {}
  return {
    resultado: '',
    gai: sanitizeCell(input.gai),
    percentil: sanitizeCell(input.percentil),
    intervaloConfianca90: sanitizeCell(input.intervaloConfianca90),
    intervaloConfianca95: sanitizeCell(input.intervaloConfianca95),
  }
}

export function sanitizeWiscResults(value: unknown): WiscResults {
  if (!value || typeof value !== 'object') return emptyWiscResults()
  const input = value as Record<string, unknown>
  const subtests: Record<string, WiscSubtestResult> = {}
  const rawSubtests =
    input.subtests && typeof input.subtests === 'object'
      ? (input.subtests as Record<string, unknown>)
      : {}

  for (const key of WISC_EVALUATION_KEYS) {
    if (!(key in rawSubtests)) continue
    subtests[key] = sanitizeSubtestResult(key, rawSubtests[key])
  }

  const scaleSummary: Record<string, WiscScaleSummaryRow> = {}
  const rawScaleSummary =
    input.scaleSummary && typeof input.scaleSummary === 'object'
      ? (input.scaleSummary as Record<string, unknown>)
      : {}

  for (const row of WISC_SCALE_SUMMARY_ROWS) {
    if (!(row.key in rawScaleSummary)) continue
    scaleSummary[row.key] = sanitizeScaleSummaryManualFields(rawScaleSummary[row.key])
  }

  const gaiSummary =
    input.gaiSummary && typeof input.gaiSummary === 'object'
      ? sanitizeGaiSummaryManualFields(input.gaiSummary)
      : emptyWiscGaiSummary()
...
export function isEmptyWiscResults(results: WiscResults): boolean {
  ...
  const hasScaleManualData = Object.values(results.scaleSummary).some(
    (row) =>
      row.qi !== '' ||
      row.percentil !== '' ||
  ...
  return !hasSubtestData && !hasScaleManualData && !hasGaiManualData
}
