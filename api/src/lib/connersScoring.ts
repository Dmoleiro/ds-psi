import normsData from './connersNorms.json' with { type: 'json' }
import type { QuestionnaireScores } from './questionnaires/types.js'
...
export type ConnersQualitative = 'Típico' | 'Ligeiramente atípico' | 'Possivelmente atípico' | 'Moderadamente atípico' | 'Marcadamente atípico'
...
export function computeConnersScores(
  variant: ConnersVariant,
  answers: Record<string, unknown>,
  itemCount?: number,
): QuestionnaireScores {
  const summary = computeConnersSummary(variant, answers, itemCount)
  const scores: QuestionnaireScores = { total_raw: summary.total.raw }

  for (const [key, row] of Object.entries(summary.subscales)) {
    scores[`${key}_raw`] = row.raw
    if (row.standardScore !== null) scores[`${key}_standard`] = row.standardScore
    if (row.percentile) {
      const ptNum = Number(row.percentile.split('-')[0])
      if (Number.isFinite(ptNum)) scores[`${key}_percentile`] = ptNum
    }
  }
